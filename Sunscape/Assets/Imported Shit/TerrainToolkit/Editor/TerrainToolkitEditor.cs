...
public class TerrainToolkitEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		switch (terrain.toolModeInt) {
			...
			// -------------------------------------------------------------------------------------------------------- TEXTURING TOOLS
			case 2:
			Terrain terTopLevel = (Terrain) terrain.GetComponent(typeof(Terrain));
			if (terTopLevel == null) {
				return;
			}
			TerrainData terDataTopLevel = terTopLevel.terrainData;
			terrain.splatPrototypes = terDataTopLevel.splatPrototypes;
			EditorGUILayout.Separator();
			float mouseX;
			EditorGUILayout.BeginHorizontal();
			GUI.skin = terrain.guiSkin;
			...
			EditorGUILayout.EndHorizontal();
			if (GUI.changed) {
				terDataTopLevel.splatPrototypes = terrain.splatPrototypes;
			}
